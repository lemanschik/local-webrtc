...
pull(c){c.enqueue(this.pc)}}).pipeThrough(new TransformStream({async transform(c,peerConnection){
  ...
  const messageHandler = {
    ...
    async ['peerConnection.ondescription'](description){ // onsignal;
        if (network.ignoreOffer = description.type === 'offer' && !polite && (makingOffer || !(peerConnection.signalingState === 'stable' ||
        (peerConnection.signalingState === 'have-local-offer' && network.srdAnswerPending)))){
          return broadcastChannel.postMessage({method: "peerconnection.ondescription", params: [{ result: { value: 'glare - ignoring offer' }}] });
        }
        network.srdAnswerPending = description.type === 'answer';
        await peerConnection.setRemoteDescription(description);
        network.srdAnswerPending = false;
        try {
          if (network.srdAnswerPending) { // negotiated: true // auto negotiated
    ...
    async ['peerConnection.onnegotiationneeded'](){
      broadcastChannel.postMessage({method: "peerconnection.onnegotiationneeded", params: [] });
      try {
        ...
        network.makingOffer = false;
      }
    },
    ['peerConnection.onicecandidate'](candiate){
      broadcastChannel.postMessage({ method: "peerconnection.onicecandidate", params: [candidate] });
  ...
  peerConnection.onicecandidate = ({candidate}) => void messageHandler["peerconnection.onicecandidate"](candidate);
  peerConnection.onnegotiationneeded = async () => void messageHandler["peerconnection.onnegotiationneeded"]();
  streamChannel.onmessage = ({data}) => c.enqueue(data);
  streamChannel.postMessage({ protocol: { sessionId: peerIdentity, id: crypto.createRANDOMUUID(), method: "peerConnection.createDataChannel", params: [] } })
  c.enqueue({ protocol: { sessionId: peerIdentity, id: crypto.createRANDOMUUID(), method: "peerConnection.createDataChannel", params: [] } });
